...
import 'package:otper_mobile/app_flowy_board/src/utils/log.dart';
import 'package:otper_mobile/app_flowy_board/src/widgets/board_group/group_data.dart';
  ...
  Widget build(BuildContext context) {
    Widget dragTarget = DragTarget<T>(
      ...
      onMove: (detail) {
        ...
        final scrollZoneWidth = widget.groupWidth * 0.3; // 30% of group width
        final leftBoundary = scrollZoneWidth;
        final rightBoundary = screenWidth - scrollZoneWidth;
        
        // Calculate scroll speed based on proximity to edge
        final distanceFromLeftEdge = detail.offset.dx;
        final distanceFromRightEdge = screenWidth - detail.offset.dx;
        
        double scrollSpeed = 0;
        
        if (distanceFromRightEdge < scrollZoneWidth) {
          scrollSpeed = (scrollZoneWidth - distanceFromRightEdge) / scrollZoneWidth * 30;
        } else if (distanceFromLeftEdge < scrollZoneWidth) {
          scrollSpeed = -(scrollZoneWidth - distanceFromLeftEdge) / scrollZoneWidth * 30;
        }
        
        if (scrollSpeed != 0) {
          final newPosition = (scrollController.offset + scrollSpeed).clamp(
